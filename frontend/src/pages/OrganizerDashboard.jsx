        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <FeatureCard
              ...
              items={bookings.length > 0 ? bookings.map(booking => {
                const ticketInfo = booking.ticketDetails && booking.ticketDetails.length > 0 
                  ? booking.ticketDetails.map(detail => `${detail.quantity}x ${detail.ticketType}`).join(', ')
                  : `${booking.tickets} ticket${booking.tickets > 1 ? 's' : ''}`;
                return `${booking.customerName} - ${ticketInfo} (EGP ${booking.totalPrice})`;
              }) : ['No bookings yet']}
